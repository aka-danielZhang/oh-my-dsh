...
- 修复 macOS 收起侧栏时会话标题被标题带图标（侧栏开关 / 下载按钮 / 通知铃铛 / 新会话）压住（bridge 0.2.0-rc.14）：此前收起态只让出红绿灯的 80px，标题文字从带内控件底下穿过；现改按控件实测右缘动态避让（ResizeObserver 发布 CSS 变量），有更新可用、通知铃铛出现/消失时标题自动跟随右移或回退，测量缺席时退化为原 80px 灯排避让。

...
## [0.3.1-rc.8] - 2026-09-17

### Fixed

- 修复「定时任务」侧栏时钟按钮在搜索展开再收起后消失的问题：搜索展开时上游会在搜索控件内部挂载瞬态清除按钮，入口的表头行定位被误判、时钟按钮被搬进裁剪不可见的搜索框；现改为展开期正确让位、收起后自动回位，已中招的安装升级重载即恢复（dsh-scheduled-tasks 0.1.3）。

### Improved

- 「定时任务」侧栏时钟的让位/回归改为 120ms 淡入淡出（与设置页按钮同一材质），收起后不再「突然出现」；`prefers-reduced-motion` 下无动画（dsh-scheduled-tasks 0.1.4）。

## [0.3.1-rc.7] - 2026-09-16

### Fixed

- 修复侧栏收起后无法再展开的问题：收起态下主内容避让用 `padding-left` 实现，工具栏主通道的盒子从 x=0 铺满整行，其继承的窗口拖拽区按绘制顺序盖住了侧栏开关按钮的点击洞——原生鼠标按下被当成拖动窗口，点击到不了页面。避让改为外边距（`margin-left:176/204px`），拖拽面不再覆盖按钮，显示位置与动画完全不变。
- 修复 e2e 探针自 0.2.0-rc.18 起中断的问题：更新入口改为按条件渲染后，探针的「工具栏至少 4 个按钮」断言永远先失败，收起/展开循环断言从未运行；同版让探针对条件渲染的按钮做可见性过滤，并在收起态新增「主通道盒子不得覆盖控件簇」与「开关保持 no-drag」两条防回归断言。rc.6 已知问题中的「工具栏按钮 3/4」即此探针假警报，随之消除。

...
## [0.3.1-rc.6] - 2026-09-16

### Fixed

- 修复 rc.5 仍无法启动的问题：上游 0.1.6 的 profile 解析强制使用 `node-addon-require-builtin` 原生模块，该模块只接受精确的 Electron 43.0.0/44.0.0/45.0.0-alpha.6 或纯 Node 22/24/26 运行时指纹——壳此前的 Electron 37（内嵌 Node 22.21.1）被直接拒绝，导致全部插件条目加载失败并在补丁监听阶段崩溃退出。壳升级并精确钉住 Electron 44.0.0（与上游桌面一致），已实机验证 sidecar 完整启动、17 个随包插件全部激活。

### Known Issues

- 标题栏会话头区域（⋯ 菜单等）在 0.1.6 基线下可能未随工具栏组合渲染，e2e 探针检测到工具栏按钮 3/4；不影响会话功能本体，将在后续版本跟进。
- `dsh-compaction-hierarchical` 对 0.1.6 压缩栈的 4 个边界语义测试红（一次性溢出回退、溢出终止、输出预留、模型上限路由；此前被本地陈旧构建产物掩盖未暴露）。插件可正常加载激活，常规压缩可用，边界行为适配随后续版本跟进。
...
## [0.3.1-rc.3] - 2026-09-13

### Fixed

- 「记忆」设置里那两次 `output hit the model's max-token ceiling` 是误报：模型把 JSON 套在 ```json 围栏里，严格解析失败后走了抢救路径，而抢救一律被标成「截断」。现在把「这轮 turn 为什么结束」「正文怎么被解析」「是否真被截断」分开记录——围栏 JSON 按格式恢复处理（剥壳后照常逐项校验证据，不再算作截断），只有 turn 真的以 `max-tokens` 结束时才叫容量截断；设置页也改为按这些字段显示结束原因、输出格式、是否完整，以及真正的截断／协议错误警告。历史记录缺分类信息时标明无法判定，不再计入截断率。随包 dsh-ohmymemo 升级到 0.3.2。

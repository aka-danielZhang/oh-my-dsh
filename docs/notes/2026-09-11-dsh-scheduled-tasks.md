...
- **十次微调（2026-09-17）：让位淡入淡出**。0.1.3 的让位是 `visibility` 瞬时翻转——收起后时钟「突然出现」，用户反馈不丝滑。改为 CSS 过渡驱动：入口标记 `data-dsh-stask-hidden`，`.dsh-stask-entry` 挂 `opacity 120ms ease, visibility 0s linear`，隐藏态把 visibility 翻转延迟到淡出播完（120ms）再裁剪，回归时立即可见并淡入；`prefers-reduced-motion` 降级为无过渡。时序实测（scratch + 真实浏览器）：展开后 ~60ms 扫描触发淡出，180ms 盒子扫到原位时图标恰好淡完；收起后 settle 重锚定再淡入 ~120ms，全程与盒子无重叠、无跳位，位置与收起态逐像素一致。插件升版 0.1.4（pin 同步）。
- **九次微调（2026-09-17）：搜索展开期入口让位与行判定加固**。用户实机报告：点开搜索框（占满整行）再收起，时钟按钮消失。两层根因——① 行判定 `rowOf` 以「最近含 ≥2 按钮祖先」定位表头行，而搜索展开时上游在搜索控件内部挂载瞬态清除按钮，使**搜索控件自身**恰好凑满 2 个按钮被误判为行，入口被搬进 `overflow: hidden` 的搜索框（负 left 裁剪）；收起后清除按钮卸载，但入口自身又替该容器凑满 2 个，从此永久卡死在里面。② 展开的搜索框 180ms 从右向左扫过整行，入口绝对定位浮在行上（`z-index:5`），盒子从它身上扫过后视觉上「落进」搜索框；扫描在展开后 60ms 测到的是过渡中途几何，之后 CSS 过渡不再触发 mutation，错位坐标就此凝固。修复三件：行判定抽纯函数 `qualifyingRowButtons`（排除自身标记按钮与搜索簇内非锚点按钮；搜索簇 = input 的**直接**父容器，避免把整行误当簇导致回退锚点被永久判为展开；DOM-free 单测 `tests/entry.test.ts` 钉死回归）；扫描检测到清除按钮（簇内既非锚点也非自身的按钮，纯结构判断不依赖文案/locale）即 `visibility: hidden` 让位——与上游自己隐藏 headerActions 同姿势；收起后等 260ms 过渡 settle 再重锚定并显示（后台标签页 setTimeout 节流只会晚到、不改变行为）。重挂载时清理旧版可能留在搜索框上的 `ROW_MARK`/内联 position（升级自愈：已卡死的安装重载即恢复，实测确认）。fork 回退路径（aria 锚失配落到添加按钮锚点）下 cluster 为 null，不参与展开判定、不误隐藏。实机四场景验证（scratch home + 真实浏览器）：收起定位 / 展开隐藏 / 收起恢复 / 卡死自愈；插件升版 0.1.3（pin 同步）。
- **八次微调（2026-09-12）：模型菜单对齐对话框**。按用户实机对照把模型芯片做成对话框同款：触发面「模型 · 强度」；根菜单两级（模型 / 推理等级），模型列表按供应商分组并显示真实模型名；切模型后强度跟随重置。数据面修正三处——① `resolveModelInfo().reasoning.efforts` 的元素是 `LlmReasoningEffortInfo { id, name }`（一度误读 `.value` 导致档位恒空）；② 模型条目补 `name`（`LlmModelInfo.name`，对话框显示的就是它，缺省回退 id）；③ 补 `defaultEffort`——未显式选择时触发器显示模型声明的默认档位（对话框里「GLM-5.3-Flash Max」的来源）。策略取舍：**推理等级行常驻**（对话框仅在模型有 reasoning 元数据时渲染该项，本插件按用户要求恒显示，无声明时值为 `Default`）；目录单次上限 40 个模型。`catalogSnapshotSchema` 的模型条目新增可选 `name` / `defaultEffort`（`.optional()`，存量客户端兼容）。预览宿主同期补防御：目录调用异常时返回目录形状 + `error` 字段（客户端兜底数组并显示错误横幅），不再让客户端 `.then` 抛错后静默停在加载态；插件升版 0.1.2（pin 同步）。

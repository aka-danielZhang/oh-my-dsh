...
- **提取**：专用 root Agent 无可见 Tools，执行 guard 拒绝全部工具调用；维护会话 cwd 恒为中性 home 目录（不指向证据 workspace——那会把该仓 AGENTS.md 拉进每次 prompt 并把维护会话归组到该 workspace，本插件会话在 UI 中呈「未分组」）。输出必须是**一个锚定的 JSON 文档**——裸 JSON，或整体被单层 ` ```json `（语言标记可空）围栏包裹；围栏属于格式变体，剥壳后仍按同一套 strict JSON、精确证据 quote 子串（≤200 字符）和 secret 检查逐项校验。任一非法 item，以及 `completed` 轮次下的畸形正文（围栏外 prose、双层围栏、非 json 标记、只剩完整项的截断前缀），都是确定性协议错误：整轮失败且不推进游标，**绝不做 salvage**。`agentMaxTokens`（默认 16384）需同时容纳最坏 JSON 输出与推理模型的思考消耗。**截断抢救**：仅当 turn 真的以 `max-tokens` 结束时，memories 数组的完整前缀才逐项通过同样的 grounding 校验并入账——截断豁免缺失的条目，绝不豁免非法的条目。**死信**：同一证据窗口（promptHash 相同）连续 `deadLetterThreshold`（默认 3）次确定性输出失败后，游标强制推进、审计标注 dead-lettered，打破「每晚重试同一毒批」循环；超时/取消/供应商错误不计入。设置页的模型/档位选择器在每次写入提交后重取 models 快照（此前已设档位会一直显示「跟随默认」，且 `min(1)` 校验使空串「清除覆盖」被拒）。产品决策（2026-09-05）：自动结果**直接写为正式记忆**——`status: active`、`privacy: normal`、`pinned: true`，立即可被 search/capsule 召回，不经候选区、无需手动确认；证据落地、append-origin 过滤、secret fail-closed、同 key 去重与 tombstone 屏障仍是护栏。游标按「seq 连续已拟合前缀」推进：时间乱序导致低 seq 证据未进 prompt 时该 Session 游标原地不动，下次 Run 重新审视（同 key 去重保证幂等）；取消若赶在提交前到达，Run 以 `cancelled` 结算且不推进游标与调度边界。
...
- **输出分类（2026-09-13）**：Run summary/audit 把三件原本被压成一个布尔值的事分开记录——`turnEndReason`（原样取自 Session 事件 `turn/end.reason.kind`）、`outputFormat`（`bare-json`/`json-fence`/`prefix-salvage`/`invalid`）、`formatRecovered`、`outputComplete`、`salvagedItems`，以及只由 `turnEndReason === 'max-tokens'` 决定的 `truncated`：**格式恢复永不冒充容量截断**。设置页按这些结构化字段渲染诊断（结束原因 / 输出格式 / 是否完整 / 是否真实截断 / 输出协议错误），不再由单一 `truncated` 生成固定的 `max-token ceiling` 文案。旧持久化记录没有 `turnEndReason`，其 `truncated` 一律按分类缺失（无法判定）展示、不计入截断率；全部新字段带 `.default()`，不挡 boot。事故核对与长期方案见 `docs/notes/2026-09-13-ohmymemo-dream-reliable-consumption.md`。
...
- 输出分类修复（阶段 A）只让观测事实可信；`docs/notes/2026-09-13-ohmymemo-dream-reliable-consumption.md` 的阶段 B–E（tool-only 显式完成协议、确定性微批、逐批 cursor、容量自愈）尚未实现——当前提取仍是「单窗口 legacy JSON + 整窗提交 cursor」路径，真实 `max-tokens` 时的前缀 salvage 仍会推进整个窗口的游标。

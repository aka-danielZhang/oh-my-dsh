...
function fakeContext(): Harness {
  ...
  return { ctx, get handler() { return handler } }
}

function fakeAgent(cwd?: string, capsules: string[] = []): FakeAgent {
  const events: Record<number, unknown> = {}
  const nodes: number[] = []
  let seq = 0
  for (const text of capsules) {
    seq += 1
    events[seq] = { type: 'user/message', data: { role: 'user', content: [{ type: 'text', text }], source: { kind: 'plugin', plugin: 'dsh-ohmymemo-context' } } }
    nodes.push(seq)
  }
  return { id: 'session-ctx-1', session: { header: { ...(cwd !== undefined ? { cwd } : {}) }, surface: { nodes }, events } }
}
...
async function step(harness: Harness, agent: FakeAgent): Promise<{ kind: string; messages: Array<{ content: Array<{ type: string; text?: string }>; source?: { plugin?: string } }> }> {
  const handler = harness.handler!
  return await handler({ agent, turn: 1, step: 1, signal: new AbortController().signal }, async () => ({ kind: 'enter', messages: [{ id: 'm0', role: 'user', content: [{ type: 'text', text: '用户直接输入' }] }] })) as never
}

test('row shape: name and inject', () => {
  assert.equal(name, 'dsh-ohmymemo-context')
  assert.deepEqual(inject, ['ohMyMemo'])
})
...
test('workspace memories rank into the capsule for the matching cwd only', async () => {
  ...
  const elsewhere = await step(harness, fakeAgent('/definitely/not/registered', []))

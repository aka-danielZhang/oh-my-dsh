...
- fork runtime `v0.1.2-alpha.5+zw.1` / `@crazx/dsh-mcp-client@0.1.2-alpha.5.zw.1`，或其他会发出 `mcp-client/status` 的 Harness 构建；官方 `dsh-mcp-client` 不发出该事件。
- Node.js `^22.19.0 || >=24`。
...
当前兼容的 DeepSeek Harness 基线：**fork `v0.1.2-alpha.5+zw.1`**。manager 会在本地镜像重连默认值与服务器名 pattern，但实时状态仍依赖 fork 的 `mcp-client/status` 事件。因此源码 checkout 的 devDependency 把 `@deepseek-ai/dsh-mcp-client` alias 到 `@crazx/dsh-mcp-client@0.1.2-alpha.5.zw.1`；生产 profile 也必须提供同样带状态事件的 peer。
